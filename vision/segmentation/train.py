...
train_path = '/scratch/npattab1/segmentation/data/train/train'
mask_path = '/scratch/npattab1/segmentation/data/masks'
# len(os.listdir(train_path)), len(os.listdir(mask_path))
df = pd.read_csv('/scratch/npattab1/segmentation/data3/stage_2_train.csv')
fs = df.loc[df['EncodedPixels'] != '-1', 'ImageId'].tolist()
...
for it in range(1):
    img_size = 512
    train_fs, val_fs = train_test_split(fs, random_state=2023, shuffle=True)
    train_ds = SegmentationDataset(train_fs, get_aug(img_size))
    val_ds = SegmentationDataset(val_fs, val_aug(img_size))
    model_name = 'swinv2_small_window8_256'
    dls = DataLoaders.from_dsets(train_ds, val_ds, batch_size=32, pin_memory=True, num_workers=8).cuda()
    model = UperNet_swin(model_name, img_size=img_size, pretrained=True)
    unet_splitter = lambda m: L(m.backbone, nn.Sequential(m.PPN, m.FPN, m.head)).map(params)
    learn = Learner(dls, model, 
                loss_func = symmetric_lovasz, 
                splitter=unet_splitter, 
                cbs = [
                    TerminateOnNaNCallback(), 
                    CSVLogger(fname=f'logs_{it}.csv', append=False),
                     EarlyStoppingCallback(monitor='dice_soft', comp=np.greater, patience=4),
                     GradientClip(3.0),
                    ImgProcCB()],
    ...
    learn.unfreeze()
    learn.fit_one_cycle(16, lr_max=1e-5)
